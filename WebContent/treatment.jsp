...
<% String a = null; %>
  ...
  <hr />
<form id="form1" name="form1" method="post" action="treat.plan?pid=<%= pid %>">
...
<td><input type="text" id="tn" name="tn" readonly />
</td>
</tr>
</table>
...
<% 
   ...
   ResultSet amal_set = stmt_alam.executeQuery("select proc_name from mine.TBLPROCEDURE where proc_name like '%amalgam%'");
%>
<td>Amal:
</td>
<td>
		<select name="amal" id="amal" >
		<option value="None">None</option>
		<%  while(amal_set.next()){ %>
		<option value="<%= amal_set.getString(1) %>"><%= amal_set.getString(1) %></option>
		<% } %> 
		<% amal_set.close();
		   stmt_alam.close();
...
<% 
   ...
   ResultSet surface_set = stmt_surface.executeQuery("select proc_name from mine.TBLPROCEDURE where proc_name like '%Surface%'");
...
<td>
		...
		<option value="<%= surface_set.getString(1) %>"><%= surface_set.getString(1) %></option>
		<% } %> 
		<% surface_set.close();
		   stmt_surface.close();
		   DBFunctions.createConnection().close();
...
<td></td>
<td>
<input type="submit" name="plan" value="Submit Plan" />
</td>
</tr>
